...
do the building can be specified with `CC_FOR_BUILD`. You can always
used `CC_FOR_BUILD`, but for native builds it is less well known because
Meson (and Autotools) will default `CC_FOR_BUILD` with `CC`.
